function [B_norm] = backward_iterative_linear(x, u, A, E, Cn)
    ...
    for t = n-1:-1:1  % For each time point from second-to-last to the first
        for state = 1:k  % For each state 1->k
            ...
            for next_state = 1:k
                B_current(next_state) = B_norm(t+1, next_state) * A(state, next_state) * linear_emit_prob(x(t+1, :), u(t+1, :), E(next_state, :));
            end
