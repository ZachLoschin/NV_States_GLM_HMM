...
function [F_norm, Cn] = forward_iterative_linear(x, u, A, E, Pi)
    ...
    for state = 1:k
        ...
        F_current(state) = Pi(k) * linear_emit_prob(x(1, :), u(1, :), E(state,:));
    end
    ...
    for t = 2:n  % For each time point 2->n
        for state = 1:k  % For each state 1->k
            ...
            F_norm(t, state) = linear_emit_prob(x(t, :), u(t, :), E(state, :)) * sum(F_current);
        end
