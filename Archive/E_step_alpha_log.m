...
function [xi, gamma, alpha_log, beta_log] = E_step_alpha_log(input, output, A, E, Pi, latent)
    ...
    alpha_log = forward_iterative_log_alpha(input, output, A, E, Pi);

    beta_log = backward_iterative_log_alpha(input, output, A, E);
    ...
    for t = 1:N-1
        ...
        for i = 1:K
            for j = 1:K
                ...
                log_xi_unnormalized(i, j) = alpha_log(t, i) + log(A(i,j)) + alpha_emit_log_prob(input(t+1, :), output(t+1, :), E(K,:)) + beta_log(t+1,j);
            end
        end
        % Normalize the xi values
        max_xi = max(max(log_xi_unnormalized));
        denom = max_xi + log(sum(sum(exp(log_xi_unnormalized - max_xi))));
        xi(:,:,t) = log_xi_unnormalized - denom;
    end
