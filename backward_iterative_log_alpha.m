function logB = backward_iterative_log_alpha(x, u, A, E)
   ...
   % Inputs:   x = Nx70 vector of observed outputs
    ...
    for t = n-1:-1:1  % For each time point from second-to-last to the first
        for state = 1:k  % For each state 1->k
            ...
            for next_state = 1:k
                values_to_sum(next_state) = logB(t+1, next_state) + log(A(state, next_state)) + alpha_emit_log_prob(x(t+1, :), u(t+1, :), E(next_state, :));
            end
