...
function logF_lse = forward_iterative_log_alpha(x, u, A, E, pi)
    ...
    for state = 1:k
        logF_lse(1, state) = log(pi(k)) + alpha_emit_log_prob(x(1, :), u(1, :), E(state,:));
    end
    ...
    for t = 2:n  % For each time point 2->n
        for state = 1:k  % For each state 1->k
            ...
            logF_lse(t, state) = logsumexp(values_to_sum)  + alpha_emit_log_prob(x(t, :), u(t, :), E(state, :)); % Logsumexp to avoid underflow
        end
    end
end
